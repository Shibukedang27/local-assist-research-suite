...
- **From-scratch correction started** — clarified that the Ollama/Qwen version is adapted pretrained software, not a from-scratch model. Added a new MLX byte tokenizer, causal transformer, deterministic synthetic corpus generator, random-initialization training loop, held-out validation, checkpoint format, and inference runner for `Local Assist Tiny`.
